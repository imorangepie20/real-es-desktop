...
- **Windows 크로스컴파일**: 리눅스에서 `cargo-xwin`(0.23)으로 `x86_64-pc-windows-msvc` 타겟 빌드 + NSIS 번들 — 절차는 README §Windows 크로스컴파일. 코드 변경 없이 성공(tray·window-state·opener 모두 Windows 호환, WebView2는 NSIS가 부트스트랩). MSI는 Windows 호스트 전용(WiX). 미서명이라 SmartScreen 경고 있음. `cc` crate의 "clang-cl compiler family detection" 경고는 무해(빌드 정상). Windows 실기 검증 대기.
